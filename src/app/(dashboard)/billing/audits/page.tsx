...
import { Eye, Info, Clock, CheckCircle2 } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-border dark:border-zinc-800/80 pb-5">
        <div>
          <h1 className="text-xl font-bold text-brand-navy dark:text-foreground tracking-tight">
            Billing Security Audit Logs
          </h1>
          <p className="text-xs text-on-surface-variant/80 font-medium">
            Review timeline of credential rotations, billing invoice downloads, and subscription package updates.
          </p>
        </div>
      </div>

      {/* Audit timeline log list */}
        ...
        <div className="divide-y divide-brand-border">
          ...
          {auditLogs.length === 0 && (
            <div className="p-12 text-center text-xs text-on-surface-variant select-none">
              No billing audit logs registered.
            </div>
          )}
        </div>
      </Card>
