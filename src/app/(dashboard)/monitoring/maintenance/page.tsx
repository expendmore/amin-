      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-border dark:border-zinc-800/80 pb-5">
        <div>
          <h1 className="text-xl font-bold text-brand-navy dark:text-foreground tracking-tight">
            Maintenance Schedule Center
          </h1>
          <p className="text-xs text-on-surface-variant/80 font-medium">
            Review scheduled platform maintenance slots, check status changes updates, and explore historical archives logs.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-5 border-brand-border bg-white dark:bg-zinc-950 flex flex-col gap-4 text-left font-sans">
          ...
          <div className="flex flex-col border border-brand-border dark:border-zinc-800 rounded-xl divide-y divide-brand-border overflow-hidden">
            {([
              { date: "June 27, 2026", task: "Next.js Build Optimization compilation checks", result: "Completed" }
            ]).map((it) => (
              <div
                key={it.task}
