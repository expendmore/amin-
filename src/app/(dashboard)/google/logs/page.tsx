...
import { SyncLogType } from "@/types/google-workspace";

export default function GoogleLogsPage() {
  ...
  const [filterType, setFilterType] = useState<SyncLogType | "all">("all");
  ...
  const handleClear = () => {
    clearLogs();
    addToast("Cleared logs timeline cache", "warning");
  };

  const getLogIcon = (type: SyncLogType) => {
    switch (type) {
      case "error":
  ...
  return (
    <div className="flex flex-col gap-6 font-sans text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-border dark:border-zinc-800/80 pb-5">
        <div>
          <h1 className="text-xl font-bold text-brand-navy dark:text-foreground tracking-tight">
            Sync Audit & Incident Logs
          </h1>
          <p className="text-xs text-on-surface-variant/80 font-medium">
            Review integration audit parameters, track errors status, and verify OAuth security handshake records.
          </p>
        </div>
      ...
      {/* Logs Index */}
      <Card className="border-brand-border dark:border-zinc-800/60 bg-white dark:bg-zinc-950 overflow-hidden">
        <div className="divide-y divide-brand-border dark:divide-zinc-800/60">
