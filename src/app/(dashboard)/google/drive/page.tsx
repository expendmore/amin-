...
export default function GoogleDrivePage() {
  ...
  const [activeTab, setActiveTab] = useState<"mydrive" | "shared" | "recent">("mydrive");
  ...
  // Compute folder/file lists
  const currentFolders = useMemo(() => {
    const list = driveFolders[activeAccountId] || [];
    return list.filter(f => {
      const matchesSearch = f.name.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesParent = f.parentFolderId === selectedFolderId;
      return matchesSearch && matchesParent;
    });
  }, [driveFolders, activeAccountId, selectedFolderId, searchQuery]);

  const currentFiles = useMemo(() => {
    const list = driveFiles[activeAccountId] || [];
    return list.filter(f => {
      ...
      if (activeTab === "recent") matchesTab = true; // show all in recent sorting later
      
      const matchesParent = activeTab === "recent" ? true : f.parentFolderId === selectedFolderId;
      return matchesSearch && matchesTab && matchesParent;
    });
  }, [driveFiles, activeAccountId, selectedFolderId, activeTab, searchQuery]);

  const activeFile = useMemo(() => {
    const list = driveFiles[activeAccountId] || [];
    return list.find(f => f.id === selectedFileId) || null;
  }, [driveFiles, activeAccountId, selectedFileId]);
  ...
  const currentFolderPath = useMemo(() => {
    ...
    return path;
  }, [driveFolders, activeAccountId, selectedFolderId]);
      ...
      setUploadProgress(prev => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
            ...
            uploadFile(
              `Simulated Upload ${Math.floor(100 + Math.random()*900)}.pdf`,
              "pdf",
              "1.2 MB",
              selectedFolderId
            );
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      case "pdf": return <File className="h-5.5 w-5.5 text-red-500" />;
      case "spreadsheet": return <FileText className="h-5.5 w-5.5 text-emerald-500" />;
      case "document": return <FileText className="h-5.5 w-5.5 text-blue-500" />;
      case "image": return <ImageIcon className="h-5.5 w-5.5 text-amber-500" />;
      default: return <File className="h-5.5 w-5.5 text-slate-400" />;
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 font-sans text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-border dark:border-zinc-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-on-surface-variant/80 font-medium">
            Upload document templates, archive checkout receipts, and sync customer attachment directories.
          </p>
        </div>
      ...
      <div className="flex border-b border-brand-border dark:border-zinc-800/80 gap-6">
        {(["mydrive", "shared", "recent"] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => {
              setActiveTab(tab);
          ...
          >
            {tab === "mydrive" ? "My Drive" : tab === "shared" ? "Shared With Me" : "Recent Files"}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[500px]">
        {/* Drive list view on the left */}
        <Card className="lg:col-span-8 border-brand-border dark:border-zinc-800/60 bg-white dark:bg-zinc-950 flex flex-col overflow-hidden">
          {/* Search bar */}
          <div className="p-3 border-b border-brand-border dark:border-zinc-800/60 flex items-center gap-2 bg-white dark:bg-zinc-950">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-brand-border dark:divide-zinc-800 max-h-[520px]">
            {/* Folders Section (Only show inside My Drive) */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {currentFolders.map((fld) => (
                    <div
                      ...
                      className="p-3 border border-brand-border hover:border-brand-navy/60 hover:bg-brand-slate/40 dark:border-zinc-800 dark:hover:bg-zinc-900/40 rounded-xl flex items-center gap-3 cursor-pointer transition-all"
                    >
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="flex flex-col min-w-0 text-left">
                            ...
                            <span className="text-[10px] text-on-surface-variant/80 font-semibold">
                              Modified {new Date(file.modifiedAt).toLocaleDateString()} • {file.size}
                            </span>
                          </div>
                        </div>
        ...
        {/* File Preview detailed Panel on the right */}
        <Card className="lg:col-span-4 p-5 border-brand-border dark:border-zinc-800/60 bg-white dark:bg-zinc-950 flex flex-col gap-4 text-left justify-between">
          {activeFile ? (
            <div className="flex flex-col gap-4 h-full">
              <div className="flex flex-col gap-3 text-left">
                <div className="flex items-center gap-2 pb-2 border-b border-brand-border dark:border-zinc-800">
                ...
                <div className="flex flex-col gap-2 text-[11px] font-semibold text-on-surface-variant">
                  <div className="flex justify-between">
                    ...
                    <span className="text-brand-navy dark:text-foreground uppercase font-bold">
                      {activeFile.type}
                    </span>
                  </div>
                  <div className="flex justify-between">
              ...
              {/* Shared with List */}
              {activeFile.isShared && (
                <div className="flex flex-col gap-1.5 border-t border-brand-border dark:border-zinc-800 pt-3 text-left">
                  <span className="text-[10px] font-bold text-on-surface-variant/50 uppercase tracking-wider block">
                    Shared Access Users
                  </span>
                  <div className="flex flex-col gap-1">
                    {activeFile.sharedWith.map((email, idx) => (
              ...
              {/* Operations triggers */}
              <div className="flex items-center gap-2 border-t border-brand-border dark:border-zinc-800 pt-4 mt-auto">
                <a
                ...
                <button
                  onClick={() => {
                    ...
                    addToast("File trashed successfully", "warning");
                    setSelectedFileId(null);
                  }}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-2">
              ...
              <span className="text-xs text-on-surface-variant/80 font-semibold">
                Choose a file to preview metadata
              </span>
            </div>
          )}
        </Card>
