...
export default function MicrosoftCalendarPage() {
  ...
  const [viewMode, setViewMode] = useState<"agenda" | "day" | "week" | "month">("agenda");
  const [selectedCalendarId, setSelectedCalendarId] = useState<string>("ms-cal-primary");
  ...
  const handleCreateEvent = (e: React.FormEvent) => {
    ...
    createEvent({
      ...
      organizer: activeAccObj?.email || "admin@workspace.com",
      status: "confirmed",
      isAllDay: false,
      isRecurring: false,
      color: "#0078D4",
      calendarId: selectedCalendarId,
      teamsMeetingUrl: enableTeams ? "https://teams.microsoft.com/l/meetup-join/auto-generated" : undefined
    });

    addToast(`Meeting scheduled successfully: ${subject}`, "success");
    setSubject("");
    setBodyPreview("");
    setStart("");
    setEnd("");
  ...
  return (
    <div className="flex flex-col gap-6 font-sans text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-brand-border dark:border-zinc-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-on-surface-variant/80 font-medium">
            Review corporate calendar schedules, set timezone properties, and configure automated Teams meeting invites.
          </p>
        </div>
        <Button
          variant="success"
      ...
      {/* Calendar body */}
      {viewMode === "agenda" ? (
        <Card className="border-brand-border bg-white dark:bg-zinc-950 flex flex-col divide-y divide-brand-border dark:divide-zinc-800">
          <div className="p-4 bg-brand-slate/40 dark:bg-zinc-900/20 flex items-center justify-between">
            ...
            <span className="text-[10px] font-semibold text-on-surface-variant/80">
              {currentEvents.length} upcoming meetings
            </span>
          </div>
          ...
          {currentEvents.length > 0 ? (
            currentEvents.map((evt) => {
              ...
              return (
                <div key={evt.id} className="p-4 flex flex-col md:flex-row md:items-start justify-between gap-4 text-left">
                  ...
                  <div className="flex-1 flex flex-col gap-1.5 text-left">
                    <h4 className="font-bold text-sm text-brand-navy dark:text-foreground flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: evt.color || "#0078D4" }} />
                      {evt.subject}
                    </h4>
                    ...
                    <div className="flex flex-wrap items-center gap-3.5 mt-1">
                      ...
                      {evt.teamsMeetingUrl && (
                        <a
                          href={evt.teamsMeetingUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="text-[10px] text-[#5B5FC7] font-bold flex items-center gap-1 hover:underline"
                        >
        ...
        <Card className="p-8 border-brand-border bg-white dark:bg-zinc-950 flex flex-col items-center justify-center text-center gap-3 min-h-[300px]">
        ...
        <div className="fixed inset-0 bg-brand-navy/40 dark:bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-lg p-6 bg-white dark:bg-zinc-950 flex flex-col gap-5 border border-brand-border">
            <div className="flex flex-col gap-0.5 text-left border-b border-brand-border pb-3">
              ...
              <p className="text-xs text-on-surface-variant/80 font-medium">
                Create new meeting blocks in your active Outlook calendar folder.
              </p>
            </div>
            ...
            <form onSubmit={handleCreateEvent} className="flex flex-col gap-4 text-left">
              <div className="flex flex-col gap-1">
                ...
                <Input
                  placeholder="e.g. ExpendMore M365 Development Sprint Sync"
                  value={subject}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant/70">
                  Attendees (Emails list, comma split)
                </label>
                <Input
                  placeholder="e.g. sales@company.com, priya@company.com"
                  value={attendees}
              ...
              <div className="flex items-center gap-2 p-3 bg-brand-slate rounded-lg border border-brand-border/40">
                ...
                />
                <label htmlFor="teams-meet" className="text-xs font-bold text-brand-navy cursor-pointer flex items-center gap-1.5">
                  <Video className="h-4 w-4 text-[#5B5FC7]" />
                  Generate Microsoft Teams video conference invite
                </label>
              </div>
