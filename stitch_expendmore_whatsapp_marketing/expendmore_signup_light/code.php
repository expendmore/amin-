            ...
            if (in_array(strtolower($password), $commonPasswords)) {
                ...
                try {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        Mailer::getInstance()->sendVerification($emailInput, $fullName, $token, true);
...
<head>
    ...
    <style>
        ...
        .glass-card:hover {
            border-color: #006d2f;
            box-shadow: 0 0 20px rgba(0, 109, 47, 0.05);
        }
        .hero-gradient {
...
<body class="overflow-x-hidden min-h-screen flex flex-col justify-between hero-gradient">
    ...
    <nav class="flex justify-between items-center w-full px-margin-desktop h-16 bg-[#ffffff]/80 backdrop-blur-xl border-b border-outline-variant">
        ...
        <div class="flex items-center gap-md">
            <a href="../expendmore_signup/code.php" class="text-on-surface-variant hover:text-primary transition-colors flex items-center gap-xs text-label-sm font-label-sm">
        ...
        <div class="w-full max-w-lg glass-card rounded-2xl p-lg relative overflow-hidden my-4">
            ...
            <div class="relative z-10 space-y-md">
                ...
                <form action="code.php" method="POST" class="space-y-md">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        <div class="space-y-xs">
                            ...
                            <input type="text" id="full_name" name="full_name" required placeholder="John Doe" class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all" value="<?php echo isset($_POST['full_name']) ? htmlspecialchars($_POST['full_name']) : ''; ?>">
                        </div>
                        <div class="space-y-xs">
                            <label for="company_name" class="text-label-sm font-label-sm text-on-surface-variant block">Company Name</label>
                            <input type="text" id="company_name" name="company_name" required placeholder="Acme Inc." class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all" value="<?php echo isset($_POST['company_name']) ? htmlspecialchars($_POST['company_name']) : ''; ?>">
                        </div>
                    </div>

                    <div class="space-y-xs">
                        <label for="email" class="text-label-sm font-label-sm text-on-surface-variant block">Business Email</label>
                        <input type="email" id="email" name="email" required placeholder="name@company.com" class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all" value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>">
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        <div class="space-y-xs">
                            <label for="password" class="text-label-sm font-label-sm text-on-surface-variant block">Password</label>
                            <input type="password" id="password" name="password" required placeholder="••••••••" class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all">
                        </div>
                        <div class="space-y-xs">
                            <label for="confirm_password" class="text-label-sm font-label-sm text-on-surface-variant block">Confirm Password</label>
                            <input type="password" id="confirm_password" name="confirm_password" required placeholder="••••••••" class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        <div class="space-y-xs">
                            ...
                            <input type="tel" id="phone_number" name="phone_number" required placeholder="+1 (555) 000-0000" class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all" value="<?php echo isset($_POST['phone_number']) ? htmlspecialchars($_POST['phone_number']) : ''; ?>">
                        </div>
                        <div class="space-y-xs">
                            <label for="country" class="text-label-sm font-label-sm text-on-surface-variant block">Country</label>
                            <select id="country" name="country" required class="w-full bg-[#fcfdf6] border border-[#e1e3db] rounded-lg py-3 px-4 text-on-surface focus:outline-none transition-all">
                    ...
                    <div class="space-y-xs">
                        <div class="flex items-start gap-xs">
                            <input type="checkbox" id="terms" name="terms" required class="mt-1 w-4 h-4 rounded bg-[#fcfdf6] border-[#e1e3db] text-primary focus:ring-0 focus:ring-offset-0">
                        ...
                        <div class="flex items-start gap-xs mt-2">
                            <input type="checkbox" id="newsletter" name="newsletter" class="mt-1 w-4 h-4 rounded bg-[#fcfdf6] border-[#e1e3db] text-primary focus:ring-0 focus:ring-offset-0" <?php echo isset($_POST['newsletter']) ? 'checked' : ''; ?>>
                    ...
                    <button type="submit" class="btn-primary w-full py-3 rounded-lg text-white font-bold text-body-md flex items-center justify-center gap-xs shadow-lg shadow-primary-container/20">
                        <span>Sign Up</span>
                ...
                <div class="relative flex py-2 items-center">
                    <div class="flex-grow border-t border-[#e1e3db]"></div>
                    <span class="flex-shrink mx-4 text-label-xs font-label-xs text-on-surface-variant/40">or continue with</span>
                    <div class="flex-grow border-t border-[#e1e3db]"></div>
                </div>

                <a href="../auth_core/google_oauth.php" class="w-full py-3 px-4 rounded-lg bg-[#fcfdf6] border border-[#e1e3db] hover:bg-[#eeeff1] transition-colors flex items-center justify-center gap-xs text-body-md font-bold text-on-surface">
